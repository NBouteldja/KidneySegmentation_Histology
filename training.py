...
from model import Custom, CustomContextDACRMP, UNetVanilla, UNetVanillaMod, CE_Net_2D, CE_Net_Inception_Variants_2D, StackedUNet, CustomContext, Custom_768_512, CustomPretrainedEncoder, CustomPretrainedEncoderCustomizedContext, CustomPretrainedEncoderCustomized
...
def set_up_training(modelString, setting, epochs, batchSize, lrate, weightDecay, logger, resultsPath):
    ...
    # 12: Healthy, Healthy_TTA, UUO, UUO_TTA, AdN, AdN_TTA, Alport, Alport_TTA, IRI, IRI_TTA, NTN, NTN_TTA
    allClassEvaluators = []
    for z in range(12):
    ...
    elif modelString == 'unet':
        model = UNetVanilla(input_ch=3, output_ch=8, modelDim=2)
    elif modelString == 'StackedUnet':
        model = StackedUNet(input_ch=3, output_ch=8, modelDim=2)
    elif modelString == 'CEnet2D':
        model = CE_Net_2D(output_ch=8)
    elif modelString == 'CE_Net_Inception_Variants_2D':
        model = CE_Net_Inception_Variants_2D(output_ch=8, inceptionBlock=False)
    else:
...
if '__main__' == __name__:
    ...
    assert(options.model in ['custom', 'unet', 'CEnet2D', 'CE_Net_Inception_Variants_2D'])
